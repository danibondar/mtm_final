...
Graph::Graph(const set<string>& vertices, const set<vector<string>>& edges) {
    ...
    for (auto it = vertices.begin(); it != vertices.end(); it++) {
        set<string> dest;
        for (auto it_2 = edges.begin(); it_2 != edges.end(); it++) {
            if ((*it_2)[0] == *it) {
                dest.insert((*it_2)[1]);
            }
        }
...
Graph Graph::operator+(const Graph& graph) const {
    map<string, set<string>> new_map = connections;
    for (const auto& connection : graph.connections) {
        auto itr = new_map.find((connection.first));
        if (itr == new_map.end()) {
            new_map.insert({connection.first, connection.second});
        } else {
            (itr->second).insert(connection.second.cbegin(), connection.second.cend());
        }
    }
...
Graph Graph::operator-(const Graph& graph) const {
    ...
    for (const auto& connection : connections) {
        auto vertex_itr = vertices.find(connection.first);
        if (vertex_itr != vertices.end()) {
            for (const auto& dest_vertex : (connection.second)) {
                if (vertices.find(dest_vertex) != vertices.end()) {
                    new_vertices.insert(dest_vertex);
                }
            }
...
Graph Graph::operator!() const {
    ...
    for (const auto& connection : connections) {
        ...
        for (const auto& vertex : vertices) {
            if (connection.second.find(vertex) == connection.second.end()) {
...
void Graph::print() {
    for (const auto& connection : connections) {
        cout << connection.first << endl;
    }
    cout << "$" << endl;
    for (const auto& connection : connections) {
        for(const auto& vertex : connection.second){
            cout <<connection.first<<" "<<vertex<< endl;
        }
    }
}
